...
def predict_transform(prediction, anchors, inp_dim=512, num_classes=7):
...
def rects_prepare(output, inp_dim=512, num_classes=7):
    prediction = None
    # transform prediction coordinates to correspond to pixel location
    for key, value in output.items():
        # anchor sizes are borrowed from YOLOv3 config file
        anchors=[]
        if key == 'layer68-conv':#'layer82-conv':
            anchors = [(116, 90), (156, 198), (373, 326)] 
        elif key == 'layer80-conv': #'layer94-conv':
            anchors = [(30, 61), (62, 45), (59, 119)]
        elif key == 'layer92-conv': #'layer106-conv':
